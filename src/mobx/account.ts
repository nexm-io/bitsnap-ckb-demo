...
import { Coins, SupportedCoins } from '../utils/supportedCoins';
...
import { coinManager } from '../utils/coin_manager';
...
const validateAddress = (
  ...
  try {
    const addressPub = coinManager.xpubToPubkey(
      accountIn.xpub,
      0,
      addressIn.index,
    );
    const derivedAddress = coinManager.deriveAddress(
      addressPub,
      accountIn.scriptType,
      accountIn.network,
    );
...
const Account = types
  ...
  .actions((self) => ({
    ...
    deriveAddress: (index: number) => {
      ...
      const addressValue = coinManager.deriveAddress(
        addressPub,
        self.scriptType,
        self.network,
      );
  ...
  .views((self) => ({
    get receiveAddress() {
      const receiveAddress = self.addresses.find(
        (addr) => addr.index === self.receiveAddressIndex,
      );
      if (receiveAddress) {
